...
def save_slice(spectra_dictionary_original, data_numpy, carving_dictionary):
    import numpy as np
    import h5py
    ...
    if fileNameHDF5_path.is_file():
        ...
        data_set = f["Data"]
        if int(spectra_dictionary_original.get("Repeat")) == 1:
        ...
        if (len(carving_dictionary[' X ']) != 1):
            for key in carving_dictionary.keys():
                if np.max(carving_dictionary[key]) != np.min(carving_dictionary[key]):
                    z_min = float(np.min(carving_dictionary[key]))
                    z_delta = float(carving_dictionary[key][1] - carving_dictionary[key][0])

            """ Calculate scaling parameteres for plot axis """
            ds.attrs['IGORWaveScaling'] = [[0, 0],
                                           [float(wide_angle / int(spectra_dictionary_original.get(" Channels: NE "))),
                                            -wide_angle / 2],
                                           [float(spectra_dictionary_original.get(" Step (FAT),eV ")),
                                            float(spectra_dictionary_original.get(" KE_start,eV "))],
                                           [z_delta, z_min]]
        else:
            ds.attrs['IGORWaveScaling'] = [[0, 0],
                                           [float(wide_angle / int(spectra_dictionary_original.get(" Channels: NE "))),
